...
- The body may be typed flush against the left margin or already indented — whichever the student does, the shallowest line becomes one level of indentation. Tab inserts four spaces, Shift+Tab removes them, Enter keeps the current indent and adds a level after a colon. `Ctrl`+`Enter` runs the tests. On-screen **Indent/Outdent buttons** do the same job on a phone, whose keyboard has no Tab key; on phones they share a row with Run that pins to the bottom of the screen, while Submit stays down in the flow so the irreversible action always costs a deliberate scroll.
- **The editor shows line numbers and highlights keywords.** Errors are reported against the lines the student typed (the fixed signature is not line 1), so the gutter number and the "line 3" in a message are the same line — and after a failed run that line is marked in the gutter. The highlighting is a `<pre>` painted behind a transparent-text textarea; the three layers only line up while their font, line height, padding and tab size are identical, which `tests/ui.test.js` enforces against the stylesheet.
